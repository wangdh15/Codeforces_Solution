...
void solve() {
	...
	REP(i, 1, k) {
		...
		int a, b;
		scanf("%s %d %d\n", op, &a, &b);
		if (op[0] == 'c') rec[i] = {0, {a, b}};
		else rec[i] = {1, {a, b}};
	}
	RREP(i, k, 1) {
		if (rec[i].fi) {
			if (UFfind(fa, rec[i].se.fi) == UFfind(fa, rec[i].se.se)) ans.push_back("YES");
			else ans.push_back("NO");
		} else {
			fa[UFfind(fa, rec[i].se.fi)] = UFfind(fa, rec[i].se.se);
		}
	}
	int n = ans.size();
	RREP(i, n - 1, 0) printf("%s\n", ans[i].c_str());
}
