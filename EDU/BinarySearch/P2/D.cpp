...
void solve() {
	cin >> m >> n;
	REP(i, 1, n) cin >> t[i] >> z[i] >> y[i];
	int l = 0, r = 1100 * 15000 + 10;
	while (l < r) {
		
		int mid = (LL)l + r >> 1;
		int cnt = 0;
		REP(i, 1, n) {
			int time_t = t[i] * z[i] + y[i];
			int tt = mid / time_t;
			cnt += tt * z[i] + min(z[i], (mid - tt * time_t) / t[i]);
			// cout << cnt << endl;
		}
		// cout << l << " " << r << " " << cnt << endl;
		if (cnt >= m) r = mid;
	...
	REP(i, 1, n) {
		int time_t = t[i] * z[i] + y[i];
		int tt = l / time_t;
		cout << max(0,min(m - cur_s, tt * z[i] + min(z[i], (l - tt * time_t) / t[i]))) << " ";
		cur_s += max(0,min(m - cur_s, tt * z[i] + min(z[i], (l - tt * time_t) / t[i])));
    }			
}
