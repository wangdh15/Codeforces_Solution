...
void eval(int u, int v) {
	tr[u].lazy = v;
	if (v <= 0) tr[u].res = tr[u].pre = tr[u].suf = 0;
	else {
		tr[u].res = tr[u].pre = tr[u].suf = (LL)(tr[u].r - tr[u].l + 1) * v;
	}
	tr[u].s = (LL)(tr[u].r - tr[u].l + 1) * v;
}

void pushdown(int u) {
	if (tr[u].lazy == -2e9) return;
	eval(u << 1, tr[u].lazy);
...
void pushup(int u) {
	tr[u].pre = max(tr[u << 1].pre, tr[u << 1].s + tr[u << 1 | 1].pre);
	tr[u].suf = max(tr[u << 1 | 1].suf, tr[u << 1 | 1].s + tr[u << 1].suf);
	tr[u].s = tr[u << 1].s + tr[u << 1 | 1].s;
	tr[u].res = max(max(tr[u << 1].res, tr[u << 1 | 1].res), tr[u << 1].suf + tr[u << 1 | 1].pre);
}

void build(int u, int l, int r) {
	tr[u].l = l;
	tr[u].r = r;
	tr[u].pre = tr[u].suf = tr[u].s = 0;
	tr[u].res = 0;
	tr[u].lazy = -2e9;
	if (l == r) return;
	int mid = l + r >> 1;
...
void modify(int u, int l, int r, int v) {
	if (tr[u].l >= l && tr[u].r <= r) {
		if (v <= 0) {
			tr[u].res = tr[u].pre = tr[u].suf = 0;
			tr[u].s = (LL)(tr[u].r - tr[u].l + 1) * v;
			tr[u].lazy = v;
		} else {
			tr[u].res = tr[u].pre = tr[u].suf = tr[u].s = (LL)(tr[u].r - tr[u].l + 1) * v;
			tr[u].lazy = v;
		}
	} else {
		pushdown(u);
