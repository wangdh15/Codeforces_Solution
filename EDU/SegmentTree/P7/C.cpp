...
int query(int u, int ll, int x) {
    if (tr[u].ma < x) return 0;
    if (tr[u].l == tr[u].r) return tr[u].l;
    int mid = tr[u].l + tr[u].r >> 1;
    pushdown(u);
    if (ll > mid) return query(u << 1 | 1, ll, x);
    int res = query(u << 1, ll, x);
    if (res) return res;
    else return query(u << 1 | 1, ll, x);
}
