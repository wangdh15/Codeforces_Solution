...
void eval(int u) {
    tr[u].lazy = !tr[u].lazy;
    tr[u].num_one = tr[u].r - tr[u].l + 1 - tr[u].num_one;
}
...
void pushup(int u) {
    tr[u].num_one = tr[u << 1].num_one + tr[u << 1 | 1].num_one;
}
...
int query(int u, int k) {
    ...
    pushdown(u);
    if (tr[u << 1].num_one >= k) return query(u << 1, k);
    else return query(u << 1 | 1, k - tr[u << 1].num_one);
}
