...
void eval(int u, LL lz1, LL lz2) {
	if (lz1 == -1) {
		tr[u].lz2 += lz2;
		tr[u].s += lz2 * (tr[u].r - tr[u].l + 1);
	} else {
		tr[u].lz1 = lz1;
		tr[u].lz2 = lz2;
		tr[u].s = (lz1 + lz2) * (tr[u].r - tr[u].l + 1);
	}
}
...
void pushdown(int u) {
	eval(u << 1, tr[u].lz1, tr[u].lz2);
	eval(u << 1 | 1, tr[u].lz1, tr[u].lz2);
	tr[u].lz1 = -1;
	tr[u].lz2 = 0;
}
...
void solve() {
	...
	REP(i, 1, m) {
		...
		if (op == 3) {
			...
			int v;
			scanf("%d", &v);
			if (op == 1) modify1(1, l + 1, r, v);
			else modify2(1, l + 1, r, v);
		}
	}
}
