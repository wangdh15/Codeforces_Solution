...
void pushup(int u) {
	...
	tr[u].s2 = tr[u << 1].s2 + tr[u << 1 | 1].s2 + tr[u << 1 | 1].s1 * (mid + 1 - tr[u].l);
}

void eval(int u, LL lz) {
	tr[u].lz += lz;
	int num = tr[u].r - tr[u].l + 1;
	tr[u].s1 += lz * num;
	tr[u].s2 += lz * num * (num + 1) / 2;
}
...
LL query(int u, int l, int r) {
	if (tr[u].l >= l && tr[u].r <= r) return tr[u].s2 + tr[u].s1 * (tr[u].l - l);
	else {
		LL res = 0;
		pushdown(u);
		int mid = tr[u].l + tr[u].r >> 1;
