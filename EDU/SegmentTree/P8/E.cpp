...
void eval(int u, int mi, int ma) {
    tr[u].mi = min(tr[u].mi, mi);
    tr[u].ma = min(tr[u].mi, tr[u].ma);
    tr[u].ma = max(tr[u].ma, ma);
    tr[u].mi = max(tr[u].mi, tr[u].ma);
}

void pushdown(int u) {
    eval(u << 1, tr[u].mi, tr[u].ma);
    eval(u << 1 | 1, tr[u].mi, tr[u].ma);
    tr[u].mi = 2e5;
    tr[u].ma = -100;
}

void modify(int u, int l, int r, int h, int tp) {
    if (tr[u].l >= l && tr[u].r <= r) {
        if (tp == 2) {
            tr[u].mi = min(tr[u].mi, h);
            tr[u].ma = min(tr[u].mi, tr[u].ma);
        } else {
            tr[u].ma = max(tr[u].ma, h);
            tr[u].mi = max(tr[u].mi, tr[u].ma);
        }
    } else {
        pushdown(u);
        if (l <= tr[u << 1].r) modify(u << 1, l, r, h, tp);
        if (r >= tr[u << 1 | 1].l) modify(u << 1 | 1, l, r, h, tp);
    }
}

int query(int u, int x) {
    if (tr[u].l == tr[u].r) return max(min(tr[u].mi, 0), tr[u].ma);
    pushdown(u);
    if (x <= tr[u << 1].r) return query(u << 1, x);
