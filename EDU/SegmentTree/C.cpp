...
// URL: https://codeforces.ml/edu/course/2/lesson/4/1/practice/contest/273169/problem/C
...
void pushup(Node &re, Node &left, Node &right) {
	re.mi = min(left.mi, right.mi);
	re.cnt = 0;
	if (re.mi == left.mi) re.cnt += left.cnt;
	if (re.mi == right.mi) re.cnt += right.cnt;
}
...
Node query(int u, int l, int r) {
	...
	if (l <= tr[u].l && r >= tr[u].r) return tr[u];
		...
		Node tmp, left, right;
		left.mi = 2e9, left.cnt = 0;
		right.mi = 2e9, right.cnt = 0;		
		int mid = tr[u].l + tr[u].r >> 1;
...
void solve() {
	...
	REP(i, 1, m) {
		...
		if (op == 1) {
			...
			Node tmp = query(1, a + 1, b);
			cout << tmp.mi << " " << tmp.cnt << endl;
		}
	}
