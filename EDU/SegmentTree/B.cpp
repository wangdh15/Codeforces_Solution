...
// URL: https://codeforces.ml/edu/course/2/lesson/4/1/practice/contest/273169/problem/B
...
void modify(int u, int x, int v) {
	...
	if (tr[u].l == tr[u].r && tr[u].l == x) tr[u].mi = v;
	else  {
...
int query(int u, int l, int r) {
	...
	if (tr[u].l >= l && tr[u].r <= r) return tr[u].mi;
		...
		int tmp = 2e9;
		int mid = tr[u].l + tr[u].r >> 1;
		if (l <= mid) tmp = min(tmp, query(u << 1, l, r));
		if (r > mid) tmp = min(tmp, query(u << 1 | 1, l, r));
		return tmp;
	}
